...
function testcase() {
  ...
  if (study.addOneStudyActivity(indelPlan, Project.Variables.study_image_id)) {
    ...
    contour.deleteContourLib(indelPlan, pv, Project.Variables.contourlib_name, false, false)
    ...
    if (isExist) {
      Log.Checkpoint(`Cancel Delete PlanLib successfully!`)
    } else {
      Log.Error(`Cancel Delete PlanLib fail!`)
    }
  } else {
    Log.Error(`Cancel Delete PlanLib fail due to register study!`)
  }
