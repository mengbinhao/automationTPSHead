  ...
  if (study.addOneRegistedStudyActivity(indelPlan, Project.Variables.study_image_id)) {
    ...
    if (contour.loadAndContourTargetAreaByBrushActivity(indelPlan, 'tar')) {
      ...
      if (indelPlan.PlanGUI.VisibleOnScreen) {
        ...
        plan.setupPoint(indelPlan, "tar", 90, 30)
        indelPlan.PlanGUI.canvas.PlanC2DViewer.Drag(420, 300, 150, 150, globalConstant.obj.delayMouseOneSecond)
        LLCollection.move_out_of_main_image.Execute(indelPlan.PlanGUI) 
        
        if (strictEqual(indelPlan.CPlanInforPanel.focusList.wItems.Item(0).Items.Count, 3) && common.comparedPicture(Regions.YANGDAZHONG_MR78_plan_choice_range_point_png, indelPlan.PlanGUI.canvas.PlanC2DViewer.Picture(), globalConstant.obj.pixelTolerance, globalConstant.obj.colourToleranceForPD)) {
