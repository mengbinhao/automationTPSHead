...
function testcase() {
  ...
  if (study.addOneRegistedStudyActivity(indelPlan, Project.Variables.study_image_id)) {
    ...
    if (contour.loadAndContourTargetAreaByBrushActivity(indelPlan, 'tar')) {
      ...
      aqObject.CheckProperty(indelPlan.PlanGUI.widget.groupBox_3.pbSetPD, "Enabled", cmpEqual, true)   
      plan.setDose(indelPlan, 50, 1000)
      aqObject.CheckProperty(indelPlan.PlanGUI.widget.groupBox_3.pbSetPD, "Enabled", cmpEqual, false)    
    } else {
