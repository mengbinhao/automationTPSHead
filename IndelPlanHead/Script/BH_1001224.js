  ...
  if (study.addOneRegistedStudyActivity(indelPlan, Project.Variables.study_image_id)) {
    ...
    if (contour.loadAndContourTargetAreaByBrushActivity(indelPlan, 'tar')) {
      ...
      if (indelPlan.PlanGUI.VisibleOnScreen) {
        ...
        indelPlan.tabWidget.qt_tabwidget_stackedwidget.tab_3.toolButton_45.ClickButton()
        utilsFunctions.delay(globalConstant.obj.delayFiveSeconds)
        const afterRedo = indelPlan.CPlanInforPanel.focusList.wItems.Item(0).Items.Count
    
        if (strictEqual(before, afterUndo) && strictEqual(before + 1, after) && strictEqual(afterRedo, after) && common.comparedPicture(Regions.YANGDAZHONG_MR78_point_png, indelPlan.PlanGUI.canvas.PlanC2DViewer.Picture(), globalConstant.obj.pixelTolerance, globalConstant.obj.colourToleranceForPD)) {
